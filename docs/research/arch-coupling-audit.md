...
| `omarchy-version` | patch | lines 20-24 read the pacman package version; ask `rpm -q tinkero` instead (landed in plan 2A as `patches/0001`; it prints `VERSION-RELEASE`, for example `4.0.4-1.fc44`). The tree's `version` file cannot be used: at `v4.0.4` it still reads `4.0.0.alpha` |
...
| `install/user/first-run/enable-user-units.sh` | enables six user units including `omarchy-migrate-notify` | replace: nothing is enabled. Phase 0 found that units enabled on `graphical-session.target` also run under GNOME (`omarchy-fcitx5` crash-looped there, 188 restarts). The five kept units ship with their `[Install]` stripped and are started by `tinkero-provision --session` |
...
| `omarchy-bar:235`, `default/bash/env-bootstrap:7-8`, `default/systemd/zram-generator.conf.d/90-omarchy.conf:8` and `install/user/mise-work.sh:14` name dropped commands in comments only (the last two were found when plan 2A was executed against GitHub's tarball) | keep; permanent allowlist entries |
