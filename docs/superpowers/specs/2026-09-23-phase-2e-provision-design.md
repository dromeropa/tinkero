...
3. Start every unit named in `$TINKERO_SHARE/provision/session-units.list` (repo: `provision/session-units.list`) whose unit file exists under `$TINKERO_UNIT_DIR` or `~/.config/systemd/user/`; a missing unit is logged and skipped. At `v4.0.4` the list is the five session units the master spec names: `omarchy-crash-watch.service`, `omarchy-sleep-lock.service`, `omarchy-recover-internal-monitor.service`, `omarchy-fcitx5.service`, `bt-agent.service`. 2F appends `tinkero-inhibit-power-key.service`. Nothing is enabled. The tree ships two more units that are not on the list: `omarchy-tailscale-receive.service` is gated on `ConditionPathExists=/usr/bin/tailscale` and inert (Tinkero packages no tailscale), and `omarchy-speaker-tuning.service` is not started by the session at all but installed and enabled under `~/.config/systemd/user/` by `omarchy-audio-tuning on` on the laptops it matches (see D4).
...
At the repo root, fetched from a tagged release URL in use, run as the desktop user. Flags: `--yes` (`-y`), `--lock FILE`. The lock is `--lock`, else `upstream.lock` beside the script (a checkout), else fetched from `https://raw.githubusercontent.com/dromeropa/tinkero/$TINKERO_REF/upstream.lock`, where `TINKERO_REF` is a variable at the top of the script that reads `master` in the repo and that the release workflow (Phase 3) rewrites to the tag when it attaches the script to a release (decision D7).

1. **Preflight**, no changes: `/etc/os-release` says `ID=fedora` and `VERSION_ID` equals the lock's `fedora`; `uname -m` is `x86_64`; `/etc/systemd/system/display-manager.service` resolves to `gdm.service`; `getenforce` is reported; `dnf repoquery --installed --queryformat '%{name} %{from_repo}\n' hyprland quickshell omedora omedora-settings` shows nothing from a repository other than `copr:copr.fedorainfracloud.org:dromero:tinkero` and no `omedora*` at all; not root. Each failure stops with one explanation.
...
4. **Second gate and user stage**: `tinkero-provision --plan`, confirm unless `--yes`, then `tinkero-provision` (with `--yes` when given). Then: log out and pick "Tinkero" at GDM.

Two gates instead of the master spec's one (decision D3): the provisioning plan can only be printed by `tinkero-provision`, which does not exist on the machine before the system stage on a first install. Both gates are the same question on a re-run, where both stages are no-ops.
...
- **D7, how `install.sh` finds the lock** (section 4).
...
- **D12, nautilus-python extensions and launcher icons are not seeded.** Upstream's `/etc/skel` carries `default/nautilus-python/extensions/*.py`; GNOME's Files loads the same directory, so they would add Omarchy actions to GNOME. The launcher icons upstream installs system-wide (`applications/icons/*`) are left for later polish; the four seeded launchers show a generic icon until then.

## 8. What can be verified now, and what cannot

Hermetic, in `./dev check` and CI: the decision table; every mode of `tinkero-provision` against a fixture payload under a temporary `HOME` with stub `dconf`, `systemctl`, `git`, `mise` and the notification commands on `PATH`; the two wrappers; `install.sh` against stub `dnf`, `sudo`, `rpm`, `getenforce`, `uname` and `tinkero-provision` with a fixture `os-release` and display-manager link; the assemble steps against the fixture tree; the gates against the real tree, with the dropped-refs allowlist at six entries.
...
- Phase 3, `tinkero-status`: `~/.local/state/tinkero/seeded.tsv` (four columns as in 2.1) and `release` for the provisioning state; `--plan` output (one decision word, a tab, the path) for conflicts, moved defaults and orphans; `done/notes-<tag>` for unread config notes; `install.sh`'s `TINKERO_REF` for the release workflow.
