...
**Status:** revision 2.1, 2026-09-21. Phase 0 spike done (`docs/research/phase-0-findings.md`, GO on Phase 1); plans 2A and 2B executed; Phase 1 done 2026-09-22 (the package set builds in `dromero/tinkero`); 2C (menu rewrite) done 2026-09-23; 2E (provisioning and install) done 2026-09-23 (design: 2026-09-23-phase-2e-provision-design.md); 2D (branding) done 2026-09-24 (design: 2026-09-23-phase-2d-branding-design.md); 2F (session integration) done 2026-09-XX (design: 2026-09-24-phase-2f-session-design.md), the first COPR build of tinkero and the VM check being its post-merge issues. Fedora 44 x86_64 is the first target.
...
6. Install `/usr/share/wayland-sessions/tinkero.desktop` (`Name=Tinkero`, `Exec=uwsm start -g -1 -e -D Hyprland hyprland.desktop`), `/usr/share/uwsm/env.d/10-omarchy` (upstream's file; it also activates mise shims for the graphical session, which is how herdr-era tools and agent stubs reach keybindings), the user systemd units (upstream's kept ones with their `[Install]` sections stripped at build time, inside the tree as well as in `/usr/lib/systemd/user`, Tinkero's drop-ins for `omarchy-fcitx5`, `bt-agent` and `omarchy-speaker-tuning`, and `tinkero-inhibit-power-key.service`; nothing is enabled) (4.9), `/usr/share/uwsm/env.d/20-tinkero` (the `DCONF_PROFILE` export, 4.9), the dconf profile `/etc/dconf/profile/tinkero` (4.9), the `omarchy.ttf` icon font and fontconfig snippet, the two lock-screen PAM variants under `/usr/share/tinkero/pam/` with `/etc/pam.d/omarchy-lock-password` and `omarchy-lock-fingerprint` as `%ghost` files, the first written by `tinkero-pam-sync` in `%posttrans` (4.8), `/usr/share/tinkero/upstream.lock`, `/usr/share/tinkero/pkgmap.tsv`, and the `tinkero-*` commands.
...
**Patches** (a diff against upstream, so a rebase cost on every bump; this is the number to keep small). Twelve files in eleven patches, all but the first and `omarchy-apply-lock` under fifteen changed lines (patch 0011, `omarchy-apply-lock`, has 19, sixteen of them the deleted password block):
...
| **Theme wallpapers** | 20 of the 92 wallpapers are flat wordmarks (18 named `backgrounds/omarchy.png`, plus flexoki-light's `2-omarchy.png` and lupine's `06-omarchy.png`): the OMARCHY wordmark in the theme's accent colour on its flat background colour, one entry in each theme's wallpaper rotation. At least six more of the 92 shipped wallpapers carry the wordmark, an upstream house mark or release artwork, in illustrated form or under other names (audit, section 11) | the 20 flat ones are regenerated at build time under the source's name with omarchy replaced by tinkero from the Tinkero wordmark and each theme's own `colors.toml` (`branding/render-wallpapers`, ImageMagick), so no per-theme artwork is needed. Illustrated ones cannot be regenerated and are deleted. A wallpaper nobody has looked at carries the verdict review, which the build refuses. Every shipped wallpaper has a row in `branding/images.tsv` (path, sha256, verdict: keep, regenerate or delete) filled in by a person looking at the image; the build fails on a wallpaper with no row, which is how a new upstream image gets looked at |
| About screen's OS line | `etc/fastfetch/config.jsonc:74` prints "Omarchy <version>" | the relocated config (4.2) is on the substitution list and reads "Tinkero <version>, built on Omarchy", which is the About screen's attribution |
| Plugin metadata | `"author": "Omarchy"` in 36 of the 37 plugin and bar-widget manifests, and the menu plugin's `name`, `displayName` and `description` | rewritten by a small JSON transform (`branding/rewrite-manifests`), not by 36 rows: author becomes "Tinkero (from Omarchy)", the menu plugin's strings say Tinkero. Plugin **ids** are identifiers and do not change; the transform replaces encoded values in the file text and verifies by parsing, so formatting is untouched |
| Menu | `learn.omarchy` (opens the Omarchy manual as a web app), `update.omarchy` | the first keeps its id (an identifier) and becomes the "Tinkero" row that opens Tinkero's README through `omarchy-launch-browser`; the second is already replaced (4.4) |
...
Phase 0 also found that the `omedora-4` COPR packages pull in `nwg-panel`, `wofi`, `playerctl` and `newt`, a second bar and launcher. The forked specs in Phase 1 dropped the two `Recommends:` on `uwsm`, which removed `wofi` and `newt`. `nwg-panel` (and `playerctl`, which it recommends) is different: Fedora's own `nwg-panel` package declares `Supplements: hyprland`, so any package named `hyprland` attracts it and no spec of ours can prevent that. The `tinkero` package therefore carries `Conflicts: nwg-panel`, which makes dnf skip the weak dependency (a weak dependency that conflicts with the transaction is dropped, not an error). `install.sh` does not disable weak dependencies globally, because Tinkero's own `Recommends:` above are wanted.
...
- **Phase 1, packages: done 2026-09-22** (`docs/superpowers/plans/2026-09-22-phase-1-copr-package-set.md`). The COPR `dromero/tinkero` exists (default retention; see 4.1); the 25 specs of 4.1 are forked into `distro/fedora/specs/`; all 25 built green for fedora-44-x86_64 on the first pass, submitted in dependency order by `bin/tinkero-copr` through the `copr-build` workflow. Milestone: `dnf install hyprland quickshell uwsm mise herdr` from the COPR resolves on Fedora 44 with `hyprland 0.56.2`, `quickshell 0.3.0^20.git28771c7`, `uwsm 0.26.5`, and pulls no `wofi` or `newt`; `nwg-panel` still arrives through Fedora's `Supplements: hyprland` until the `tinkero` package's `Conflicts: nwg-panel` ships (section 6).
...
| Lock PAM file | RPM-owned `%ghost`, one of two shipped variants chosen by `tinkero-pam-sync` from what the host's `password-auth` already does (2F D6); includes `password-auth`, not `system-auth` (R2) | Keeps upstream's lockout without double counting; keeps fingerprint out of the password service; clean removal |
...
tinkero/
  ...
  systemd/{bt-agent,omarchy-speaker-tuning}.service.d/tinkero.conf
  session/uwsm-env.d/20-tinkero    the DCONF_PROFILE export (4.9)
  ...
  distro/fedora/
    ...
    skills/host.md
  ci/                              the gates (six), check-rpm, lint, VM smoke test
