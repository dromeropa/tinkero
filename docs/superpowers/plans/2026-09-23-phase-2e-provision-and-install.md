...
Recorded by the one PR that implemented this plan (issue #16), per task. Each task was reviewed on its own; where the review found a defect in the plan's own code, the fix and its tests are listed here, since the squash merge collapses the fix commits.

- **Task 2, `host.md`:** the bullet on PAM no longer says the menu's fingerprint setup entry already goes through `authselect`; `omarchy-setup-security-fingerprint` is upstream's until plan 2F (it is the arch-leak allowlist's remaining non-permanent entry). The bullet says the entry is ported in 2F and must not be run on this host until then.
- **Task 3, `tinkero-provision`:** three robustness fixes to the plan's code, each reproduced under a temporary home. A failed write inside `apply_plan` now joins `FAILED` as `file:<path>` and the loop continues, so `seeded.tsv` still records what was written (design 2.1; the plan's code aborted before `save_rows` and the written files became permanent conflicts). `hash_of` returns `other` for an existing path that is neither a regular file nor a symlink, so a directory in the way decides like a symlink (`conflict`, `keep-user` or `moved`; never written into, never deleted); the plan's code moved the temp file into the directory and wrote a row with an empty hash that broke every later run. `--reset` and `--reset-all` skip a symlinked or non-regular target with a message instead of dying, so `--reset-all` restores the rest. Small fixes in the same round: a missing or empty `~/.bashrc` is created with only the guarded line; the diff commands in the conflict and moved messages are shell-quoted; the bashrc prompt tolerates EOF. Tests: block 8b of `tests/test-provision.sh`; the suite is at `1..102` after Task 3 (the plan said `1..79`).
- **Task 4, `tinkero-provision`:** four fixes, each reproduced. `theme-in-session` is marked only when `theme.name` was empty before `theme.sh` ran, so a theme staged headless at install time is applied for real even when the first in-session run is a full provision. `step_skills` guards every `mkdir` and `ln` with `|| return 1`, and `provision` guards its state writes (`mkdir`, the log, `save_rows`, `release`) with `|| die`: bash ignores errexit inside `run_step`'s `if ( ... )` and under `session_mode`'s `provision skip || rc=1`, and no subshell form restores it, so failures there must be explicit. `session_mode` seeds dconf before the in-session steps on the current-release path, matching `run_steps`. `--remove` filters `~/.bashrc` into a temp file and writes it back through the path, so a symlinked `~/.bashrc` stays a symlink with its mode, and the filter matches the whole guarded line rather than any line with the tag. `--remove` lists `orphaned` rows whose target still exists as kept. A second round fixed the first: `step_theme`'s in-session branch ended with `[[ ... ]] && mark`, which made the step fail whenever the theme already existed. Tests: block 11b; the suite is at `1..170` after Task 4 (the plan said `1..130`).
- **Task 6, `install.sh`:** `confirm` takes an optional stop message, so declining the second gate says the tinkero package is installed and to run `tinkero-provision` when ready, instead of the plan's "nothing was changed", which is false by then. Small fixes in the same round: an empty `from_repo` is reported as an unknown source; the prompt tolerates EOF; the plan text names the COPR repository file under `/etc`; the architecture, Omedora and root refusals assert their messages. Design D3's "two prompts in total" is amended in the design document: those are Tinkero's two prompts, and dnf's own confirmations stay in an interactive run (`-y` silences them only with `--yes`), which is what design item 3 and the script already did. The suite is at `1..27` after this task (the plan said `1..21`).
- **Task 7, dates:** the `2026-09-XX` placeholders are `2026-09-23`, the date the branch was pushed; the merge date is the operator's.
- **Landing:** the branch was pushed without opening a PR, per the dispatch's interim rule (the operator opens and lands it). `rpmspec` and `rpmlint` are not installed on the build host; the spec render, parse and lint are CI's. Design section 8's host-only items are recorded on the issue, not proven.
