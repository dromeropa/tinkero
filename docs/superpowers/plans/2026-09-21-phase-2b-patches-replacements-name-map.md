...
- Final-review fix wave, F1: `patches/0010-menu-package-guards-rpm.patch` regenerated so the guard batch's `while ... done < /usr/share/tinkero/pkgmap.tsv` read gains `2>/dev/null || :`, the same defence upstream's own readers use a few lines later, so a missing or unreadable name map does not abort the guard batch under `errexit` (it just leaves every name absent). `tests/test-menu-guards.sh` gained a case sourcing the prelude with a nonexistent map path under `set -e`.
- Final-review fix wave, F2: `distro/fedora/lib/pkg.sh`'s `elevate()` compares `${TINKERO_EUID:-$EUID}` with `[[ == 0 ]]` (a string comparison) instead of `(( euid == 0 ))` (arithmetic), so an odd value in the environment cannot be evaluated as an expression; a comment now also notes pkexec runs the command in `/` with a scrubbed environment, so everything needed is passed as arguments. No test change; `tests/test-replacements.sh` stays 1..32 green on its own (it already covers root, pkexec and sudo) before F4 adds one more case.
- Final-review fix wave, F3: `ci/gate-name-map` now refuses a name map with CRLF line endings (`grep -q $'\r'`), exiting 2 with a message naming the map, rather than falling through and misreporting it as a malformed row. `tests/test-gates.sh` gained a case with a CRLF-terminated row asserting exit code 2.
- Final-review fix wave, F4: `distro/fedora/replacements/omarchy-pkg-add`'s flatpak branch now distinguishes flatpak itself being absent (`command -v flatpak` fails: "flatpak is not installed; 'dnf install flatpak' first, then add Flathub.") from flatpak being present but Flathub not configured (existing message, unchanged). `tests/test-replacements.sh` gained a case that hides the `flatpak` stub and asserts the new message.
